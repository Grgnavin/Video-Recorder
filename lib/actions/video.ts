...
const THUMBNAIL_CDN_URL = BUNNYY.CDN_URL;
...
export const saveVideoDetails = withErrorHandling(async(videoDetails: VideoDetails) => {
    ...
    await db.insert(videos).values({
        ...videoDetails,
        videoUrl: `${BUNNYY.EMBED_URL}/${BUNNY_LIBRARY_ID}/${videoDetails.videoId}`,
        userId,
        createdAt: new Date(),
        updatedAt: new Date()
    });
...
export const getTranscriptByVideoId = withErrorHandling(async (videoId: string) => {
  const url = `${BUNNYY.TRANSCRIPT_URL}/${videoId}/captions/en-auto.vtt`;
  console.log("Transcript fetch URL:", url);
